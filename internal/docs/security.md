...
The normal Windows GUI controller binds only to `127.0.0.1:7318` and rejects state-changing requests from other browser origins. Auto-start is gated on a native WPF `window-ready` signal. The WPF state view may refresh frequently, but the supplemental native-host heartbeat is emitted only once every two minutes; missing it for 2.5 minutes is logged as stale and five minutes of sustained loss stops the launcher-managed DevRelay/provider runtime through the same stop path used by the visible Start/Stop control. The actual WPF host process/window lifecycle remains authoritative and closes the controller immediately when the GUI truly exits. WebView2 renderer failures are recovered independently without stopping the runtime. Closing the GUI host invokes the stop path and then closes the controller. There is no tray/background-only mode in the normal launcher.
