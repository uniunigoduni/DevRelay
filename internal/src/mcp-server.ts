...
export function createDevRelayServer(manager: ProcessManager): McpServer {
  ...
  server.registerTool(
    ...
    async (input) => {
      try {
        ...
        return await contentResult(value, input.images, input.cwd ?? process.cwd());
  ...
  server.registerTool(
    ...
    async ({ processId, cursor, maxChars, waitMs, images }) => {
      try {
        ...
        return await contentResult(value, images, value.process.cwd ?? process.cwd());
      } catch (error) { return errorResult(error); }
    }
  );
  ...
  server.registerTool(
    ...
    async ({ processId, data, end, columns, rows }) => handled(() => {
      ...
      return manager.write(processId, data, end, resize);
    })
  );
