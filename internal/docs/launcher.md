...
- Settings stay hidden until the gear button is pressed. The theme can change while running; connection and device-name settings require the server to be stopped. Device Settings show the editable name, generated default name, aliases, immutable node ID, and local online/offline state.
